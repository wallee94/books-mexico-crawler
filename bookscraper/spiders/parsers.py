...
def parse_details_educal(response):
    ...
    data = {
        "url": response.url,
        "title": clean_text(
            response.selector.xpath('//div[@class="col-md-6 info-panel"]/span[@class="title"]/text()').extract_first()),
        ...
        "price": clean_price(
            response.selector.xpath('//div[@class="col-md-6 info-panel"]//div[@class="price"]/text()').extract_first()),
